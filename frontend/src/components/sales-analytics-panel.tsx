...
export function SalesAnalyticsPanel({ data, showManagers = false }: { data: SalesAnalytics; showManagers?: boolean }) {
  ...
  return <div>
    <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-[10px] font-semibold uppercase tracking-[.18em] text-terracotta">Все продажи</p><h1 className="mt-2 font-serif text-4xl tracking-[-.04em]">Аналитика продаж</h1><p className="mt-3 text-sm text-black/50">Выручка и валовая прибыль считаются по подтверждённым оплатам.</p></div><span className="rounded-full bg-emerald-50 px-3 py-2 text-xs font-semibold text-emerald-700">Оплачено {data.paidOrders} из {data.orders}</span></div>
    {data.paidOrdersMissingCost > 0 && <p className="mt-4 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-xs text-amber-800">Для {data.paidOrdersMissingCost} оплаченных заказов неизвестна закупочная стоимость. Они пока не включены в расчёт прибыли.</p>}
    <section className="mt-7 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">{[
      ['Выручка', money(data.revenueKopecks), 'Подтверждённые оплаты'],
      ['Валовая прибыль', money(data.grossProfitKopecks), `Себестоимость ${money(data.purchaseCostKopecks)}`],
