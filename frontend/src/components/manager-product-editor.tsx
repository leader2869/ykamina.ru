...
export function ManagerProductEditor({ productId, categories, onClose }: { productId?: string; categories: AdminCategory[]; onClose: () => void }) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch(productId ? `/api/manager/products/${productId}` : '/api/manager/products', { method: productId ? 'PATCH' : 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({
