...
export async function Header() {
  ...
  return <header className="sticky top-0 z-40 border-b border-ink/10 bg-white/95 backdrop-blur-md">
    <div className="container-page flex h-[72px] items-center justify-between gap-6">
      ...
      <div className="flex items-center gap-1 text-sm font-medium"><SavedCollections /><CartLink /><Link href="/account" aria-label="Личный кабинет" title="Личный кабинет" className="grid h-10 w-10 place-items-center rounded-full text-ink transition hover:bg-terracotta/10 hover:text-terracotta"><span className="h-5 w-5"><AccountIcon /></span></Link><a className="ml-2 hidden whitespace-nowrap text-[13px] transition hover:text-terracotta lg:block" href="tel:+74951234567">+7 (495) 123-45-67</a></div>
    </div>
  </header>;
}
