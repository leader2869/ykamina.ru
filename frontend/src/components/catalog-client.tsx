...
const initialFilters: CatalogFilters = { minPrice: '', maxPrice: '', inStock: false, width: '', height: '' };
const pageSize = 24;
...
export function CatalogClient({ products }: { products: Product[] }) {
  ...
  const filtered = useMemo(() => products.filter((product) => {
    ...
    return (!min || product.price >= min) && (!max || product.price <= max) && (!filters.inStock || isAvailable(product)) && matchesDimension(product.dimensionsData?.width, filters.width) && matchesDimension(product.dimensionsData?.height, filters.height);
