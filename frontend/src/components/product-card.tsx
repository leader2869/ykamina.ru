...
export function ProductCard({ product, badge }: { product: Product; badge?: string }) {
  return <article className="group flex h-full flex-col overflow-hidden rounded-xl border border-[#e9e5df] bg-white transition duration-300 hover:-translate-y-1 hover:shadow-card">
    ...
    <div className="flex flex-1 flex-col p-5"><p className="text-[10px] font-semibold uppercase tracking-[.16em] text-terracotta">{product.type}</p><Link href={`/catalog/${product.id}`} className="mt-2 block min-h-[4.25rem] font-serif text-[21px] font-semibold leading-[1.05] tracking-[-.025em] text-ink transition hover:text-terracotta"><span className="line-clamp-2">{product.name}</span></Link>{product.article && <p className="mt-3 text-[11px] text-ink/45">Артикул: {product.article}</p>}<p className="mt-1 min-h-5 text-xs text-ink/55">{product.dimensions}</p><div className="mt-3"><DolyameInline firstPayment={formatPrice(product.price * .25)} /></div><div className="mt-auto flex items-end justify-between gap-3 border-t border-[#eeeae4] pt-4"><div><p className="font-semibold text-ink">{formatPrice(product.price)}</p>{product.oldPrice && <p className="text-xs text-ink/40 line-through">{formatPrice(product.oldPrice)}</p>}</div><AddToCartButton productId={product.id} className="rounded-full bg-ink px-4 py-2 text-xs font-medium text-white transition hover:bg-terracotta">В корзину</AddToCartButton></div></div>
  </article>;
}
